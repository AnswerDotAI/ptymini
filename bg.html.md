...
extracted from bgterm in the first place; `PollResult` and every
function signature are unchanged from the original package.
...
The sid table and functional API, unchanged from bgterm — the sid is how
a kernel tool names a session across calls:
...
The whole flow, as a reader would use it — start a REPL-ish child, poll
for its banner, send input, read what arrived. Everything here is a
plain sync call:
